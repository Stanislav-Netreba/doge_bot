...
module.exports = async (ctx, num, uah, usd, rub, del_message_id, coin_name) => {
    ...
    const sentMsg = await ctx.reply(
        `${num} ${coin_name} → rub: ${(rub_price * num).toFixed(3)}₽ \n` +
            `${num} ${coin_name} → uah: ${(uah_price * num).toFixed(3)}₴ \n` +
            `${num} ${coin_name} → usd: ${(usd_price * num).toFixed(3)}$ \n` +
            '➖➖➖➖➖➖➖➖➖➖➖➖➖\n' +
            `Price change \n` +
            `1hour:    ${price_change_1h < 0 ? '🔴' : '🟢'}${Math.abs(
                price_change_1h
            ).toFixed(3)}% \n` +
            `1day:      ${price_change_24h < 0 ? '🔴' : '🟢'}${Math.abs(
                price_change_24h
            ).toFixed(3)}% \n` +
            `7days:    ${price_change_7d < 0 ? '🔴' : '🟢'}${Math.abs(
                price_change_7d
            ).toFixed(3)}% \n` +
            `14days:  ${price_change_14d < 0 ? '🔴' : '🟢'}${Math.abs(
                price_change_14d
            ).toFixed(3)}% \n` +
            `1month: ${price_change_30d < 0 ? '🔴' : '🟢'}${Math.abs(
                price_change_30d
            ).toFixed(3)}% \n` +
            `1year:     ${price_change_1y < 0 ? '🔴' : '🟢'}${Math.abs(
                price_change_1y
            ).toFixed(3)}% \n` +
            '\nAuthor: @Stas_Netreba \nОткрыт для заказов'
    );
